...
crate trait HirTypeChecker<F: TypeFamily>: Sized {
    ...
    fn record_ty(&mut self, index: impl hir::HirIndex, ty: ty::Ty<F>);

    /// Lookup the type for a variable.
    fn variable_ty(&mut self, var: hir::Variable) -> ty::Ty<F>;

    /// Given some permissions supplied by the user (which may be a "default"),
    /// apply them to `place_ty` to yield a new type.
    fn apply_user_perm(&mut self, perm: hir::Perm, place_ty: ty::Ty<F>) -> ty::Ty<F>;
    ...
    fn require_boolean(&mut self, expression: hir::Expression, value_ty: ty::Ty<F>);
    ...
    fn with_field(
        ...
        op: impl FnOnce(&mut Self, Self::FieldId) -> ty::Ty<F> + 'static,
    ) -> ty::Ty<F>;
    ...
    fn with_method(
        ...
        op: impl FnOnce(&mut Self, Self::MethodId) -> ty::Ty<F> + 'static,
    ) -> ty::Ty<F>;

    /// Returns a type used to indicate that an error has been reported.
    fn error_type(&mut self) -> ty::Ty<F>;

    /// Type-check `expression`, recording and returning the resulting type (which may be
    /// an inference variable).
    fn check_expression(&mut self, expression: hir::Expression) -> ty::Ty<F> {
        let ty = self
    ...
    /// an inference variable).
    fn check_place(&mut self, place: hir::Place) -> ty::Ty<F> {
        let ty = self
    ...
    fn compute_place_ty(&mut self, place: hir::Place) -> Result<ty::Ty<F>, ErrorReported> {
